"""
Facebook Celery Tasks — Async extraction and download workers.

Run the worker with:
    cd /root/FreeDownloader/fb_downloader
    celery -A tasks worker --concurrency=8 --pool=prefork -Q fb_downloader_queue --loglevel=info
"""
...
celery = make_celery('fb_downloader')
...
def _get_app_module():
    ...
    spec = importlib.util.spec_from_file_location("fb_app", os.path.join(BASE_DIR, 'app.py'))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


@celery.task(bind=True, name='fb.download_video', max_retries=1)
def celery_download_fb(self, video_url, res_height, title):
    task_id = self.request.id
    safe_title = re.sub(r'[\\/*?:"<>|]', "", title).strip() or "Facebook_Video"
    ...
    try:
        ...
        app_mod = _get_app_module()
        user_message = app_mod.classify_download_error(str(e), "Facebook")
