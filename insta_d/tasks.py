"""
Instagram Celery Tasks — Async extraction and download workers.

Run the worker with:
    cd /root/FreeDownloader/insta_d
    celery -A tasks worker --concurrency=8 --pool=prefork -Q insta_d_queue --loglevel=info
"""
...
def _get_app_module():
    ...
    spec = importlib.util.spec_from_file_location("insta_app", os.path.join(BASE_DIR, 'app.py'))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


@celery.task(bind=True, name='insta.download_video', max_retries=1)
def celery_download_insta(self, video_url, res_height, title):
    task_id = self.request.id
    safe_title = re.sub(r'[\\/*?:"<>|]', "", title).strip() or "IG_Video"
    ...
    try:
        ...
        app_mod = _get_app_module()
        user_message = app_mod.classify_download_error(str(e), "Instagram")
