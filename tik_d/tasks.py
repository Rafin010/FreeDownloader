"""
TikTok Celery Tasks — Async extraction and download workers.

Run the worker with:
    cd /root/FreeDownloader/tik_d
    celery -A tasks worker --concurrency=8 --pool=prefork -Q tik_d_queue --loglevel=info
"""
...
def _get_app_module():
    ...
    spec = importlib.util.spec_from_file_location("tik_app", os.path.join(BASE_DIR, 'app.py'))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


@celery.task(bind=True, name='tik.download_video', max_retries=1)
def celery_download_tik(self, video_url, format_str, title):
    task_id = self.request.id
    safe_title = re.sub(r'[\\/*?:"<>|]', "", title).strip() or "TikTok_Video"
    unique_filename = f"TT_Video_{uuid.uuid4().hex[:8]}.mp4"
    ...
    try:
        ...
        app_mod.download_with_retry(video_url, filepath, format_str)
        ...
        app_mod = _get_app_module()
        user_message = app_mod.classify_download_error(str(e), "TikTok")
